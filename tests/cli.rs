...
use sqli::{config::{ConfigManager, Connection}, sql::interface::SQLType};
...
fn test_case_insensitive_lookup() {
    ...
    let user_dir = temp_dir.path().join("user_config");
    let cwd_dir = temp_dir.path().join("cwd_config");
    
    std::fs::create_dir_all(&user_dir).unwrap();
    std::fs::create_dir_all(&cwd_dir).unwrap();
    
    let fs = sqli::file::FileSystem::with_paths(user_dir, cwd_dir).unwrap();
    let mut config_manager = ConfigManager::with_filesystem(fs);
    
    let conn = Connection {
        name: "PostgreSQL".to_string(),
        conn: SQLType::Postgresql,
    ...
    config_manager.add_connection(conn).unwrap();
    
    let result = config_manager.get_connection("postgresql").unwrap();
    assert!(result.is_some());
    assert_eq!(result.unwrap().name, "PostgreSQL");
    
    let connections = config_manager.list_connections().unwrap();
    assert_eq!(connections.len(), 1);
    assert_eq!(connections[0], "PostgreSQL");
}