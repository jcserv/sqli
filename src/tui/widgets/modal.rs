...
impl Widget for DialogButton<'_> {
    ...
    pub fn handle_tab(&mut self, reverse: bool) -> FocusableArea {
        match self.focused_area {
            FocusableArea::Content(idx) => {
                ...
                } else if idx < self.content_element_count - 1 {
                    self.focused_area = FocusableArea::Content(idx + 1);
                } else if !self.buttons.is_empty() {
...
impl<W> Widget for ModalDialog<'_, W>
where
    W: Widget,
{
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let content_rect = Block::default()
            .title(self.content.title)
            .borders(Borders::ALL)
            .inner(modal_area);
            
        for y in content_rect.top()..content_rect.bottom() {
            for x in content_rect.left()..content_rect.right() {
